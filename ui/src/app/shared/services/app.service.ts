...
export class AppService {
	...
	public async authenticate(
		...
		console.log("into authenticate service...", email);
		// return Promise.resolve(true);

		StorageHelper.removeToken();

		const url =  Endpoint.AUTH + Endpoint.LOGIN;
		console.log("url:::", url);		
		const { data } = await this.api.post(url, { email, password });
		
		
		console.log("data from authentication", data);
		if (!data) return false;

		const authResponse1 = new AuthResponse(data);
		StorageHelper.setToken(authResponse1);
		this.initAuthHeader();
		return true;
	}
	...
	public initRequestInterceptor(instance: AxiosInstance): void {
		instance.interceptors.request.use(
			...
			(error) => {
				console.log("interceptors.request.error", error.response.data.message);
	...
	public initResponseInterceptor(instance: AxiosInstance): void {
		instance.interceptors.response.use(
			...
			async (error: AxiosError) => {
				console.log("interceptors.response.error", Promise.reject(error));
